...
```text
./
├── meetup-scheduler-local.json   # Project-specific overrides (top-level for visibility)
├── .meetup-scheduler/
...
**Project-level venue aliases** (`meetup-scheduler-local.json`):
...
2. Support `MEETUP_SCHEDULER_ACCESS_TOKEN` environment variable override
3. Support reading token from file path via `MEETUP_SCHEDULER_TOKEN_FILE` env var
